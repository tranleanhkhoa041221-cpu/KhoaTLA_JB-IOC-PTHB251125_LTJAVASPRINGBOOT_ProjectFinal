...
public class UserServiceImpl implements UserService {
    ...
    public UserResponse updateUserStatus(Long id, UserUpdateStatusRequest request) {
        ...
        if (assessmentResultRepository.existsByEvaluatedBy_UserId(user.getUserId())) {
            throw new IllegalStateException(
                    "Không thể thay đổi trạng thái User ID = " + user.getUserId() + " vì đã tồn tại dữ liệu đánh giá");
        }
...
//        user.setIsActive(request.getIsActive());
    ...
    public UserResponse updateUserRole(Long id, UserUpdateRoleRequest request) {
        ...
        if (user.getRole() == UserRole.ADMIN && !user.getUserId().equals(id)) {
            throw new IllegalStateException("ADMIN không được phép thay đổi vai trò của ADMIN khác");
        }
        ...
        if (assessmentResultRepository.existsByEvaluatedBy_UserId(user.getUserId())) {
            throw new IllegalStateException(
                    "Không thể thay đổi role của User ID = " + user.getUserId() + " vì đã tồn tại dữ liệu đánh giá");
        }
...
//            if(user.getAssessmentResults() != null && !user.getAssessmentResults().isEmpty()) {}
            throw new IllegalStateException("Không thể xóa User ID = " + user.getUserId() + " vì đã liên kết với AssessmentResult");
        }
